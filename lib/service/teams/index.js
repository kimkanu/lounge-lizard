...
const { authenticate } = require('./teams-auth-helper');
...
class TeamsService extends Service {
  ...
  async exchangeToken(teamInput, emailInput, passInput, loginButton) {
    ...
    if (fieldsValid) {
      const access_token = await authenticate(emailInput.getText(), passInput.getText())
      return access_token
      // do something with this token, 
      // i think we should only need it for a few things (getting user teams and such) before we find 
      // alternatives for recurring requests such as messaging that would ultimately be rate limited
    }
    return null
  }
}
